...
namespace NetClient.WebSocket
{
	...
	public class MsgBody
	{
		...
		public void SetValues()
		{
			try
			{
				PacketLengthByte = WebSocketConnent.SubByte(Source, 0, 4);
				HeaderLengthByte = WebSocketConnent.SubByte(Source, 4, 6);
				ProtocolVersionByte = WebSocketConnent.SubByte(Source, 6, 8);
				OperationByte = WebSocketConnent.SubByte(Source, 8, 12);
				SequenceIdByte = WebSocketConnent.SubByte(Source, 12, 16);
				BodyDataByte = WebSocketConnent.SubByte(Source, 16, BodyDataByte.Length);

				End = WebSocketConnent.SubByte(Source, Source.Length - 2, 1)[0];
